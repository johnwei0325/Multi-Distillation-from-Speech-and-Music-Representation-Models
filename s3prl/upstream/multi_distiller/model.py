...
class MultiDistillerConfig:
    ...
    def __init__(self, config: dict):
        ...
        self.seperate_feature = config.get("seperate_feature")
        self.fbank_mean = 0
        self.fbank_std = 0
        self.hubert_weight = config.get('hubert_weight') 
        self.wavlm_weight = config.get('wavlm_weight')
        self.emotion2vec_weight = config.get('emotion2vec_weight')
...
class MultiDistillerModel(nn.Module):
    ...
    def __init__(self, config: MultiDistillerConfig):
        ...
        if config.out_layer_type == "expand-last":
            ...
            #print('gignirngirnig',config)
            if config.translator_type == 'no_translator':
                self.output_layers = nn.Sequential(
                    nn.Linear(final_emb_size, inter_dim * self.n_tasks),
                    nn.GELU(),
                    SplitLinear(inter_dim, self.n_tasks, config.final_dim),
                )
            elif config.translator_type == 'linear_on_hubert_and_conv_on_mert':
                 self.output_layers = nn.ModuleDict({
                    ...
                    if teacher == 'mert_v0_public' else nn.Sequential(
            ...
            elif config.seperate_feature:
                #print(final_emb_size)
                self.output_layers = nn.ModuleDict({
                    teacher: nn.Sequential(
                        nn.Linear(int(final_emb_size / 2), inter_dim * self.n_tasks),
                        nn.GELU(),
                        SplitLinear(inter_dim, self.n_tasks, config.final_dim),
                    )
                for teacher in self.teacher_names
                })
            elif config.translator_type == 'lconv' and not config.seperate_feature and not config.layerwise_translator:
                self.output_layers = nn.ModuleDict({
                    teacher: nn.Sequential(
                        nn.Linear(final_emb_size, inter_dim * self.n_tasks // 2),  # Linear projection for each teacher
                        nn.GELU(),  # Non-linear activation
            ...
            elif config.translator_type == 'none' and not config.layerwise_translator and not config.seperate_feature:
                self.output_layers = nn.ModuleDict({
                    teacher: nn.Sequential(
                        nn.Linear(final_emb_size, inter_dim * self.n_tasks),
                        nn.GELU(),
            ...
            elif config.translator_type == 'lconv' and config.layerwise_translator and not config.seperate_feature:
                self.output_layers = nn.ModuleDict({
            ...
            elif config.translator_type == 'none' and config.layerwise_translator and not config.seperate_feature:
                self.output_layers = nn.ModuleDict({
                    teacher: nn.ModuleDict({
                        f"task_{task}": nn.Sequential(
    ...
    def forward(self, wave, pad_mask, task_id=None, get_hidden=False, no_pred=False):
        ...
        if not no_pred:
            if self.task_emb_type == "self-hidden":
                ...
                pred = {}
                if self.config.translator_type == 'no_translator':
                    temp = self.output_layers(hidden).reshape(b_sz, t_sz, -1)
                    for teacher in self.teacher_names:
                        pred[teacher] = temp
                    #print(pred)
                elif self.config.layerwise_translator:
                    for teacher in self.teacher_names:
                        layer_preds = []
                    ...
                    for teacher in self.teacher_names:
                        if self.config.seperate_feature:
                            half_size = hidden.shape[-1] // 2
                            if teacher == "hubert_base":
                                pred[teacher] = self.output_layers[teacher](hidden[:, :, :half_size]).reshape(b_sz, t_sz, -1)
                                #print(pred[teacher].shape, b_sz, t_sz)
                            elif teacher == "mert_v0_public":
                                pred[teacher] = self.output_layers[teacher](hidden[:, :, half_size:]).reshape(b_sz, t_sz, -1)
                        else:
                            #print(hidden.shape, self.config.encoder_embed_dim)
