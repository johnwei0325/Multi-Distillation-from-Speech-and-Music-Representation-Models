...
def disable_wavlm_encoder_dropout(wavlm_model):
    """
    Disables all dropout layers in the WavLM encoder.

    Args:
        wavlm_model: The WavLM model instance whose encoder dropout needs to be disabled.
    """
    # Disable dropout in the feature projection
    if hasattr(wavlm_model.feature_projection, "dropout"):
        wavlm_model.feature_projection.dropout.p = 0.0
        print("[disable_wavlm_encoder_dropout] - Disabled dropout in feature projection.")

    # Disable dropout in the encoder
    if hasattr(wavlm_model.encoder, "dropout"):
        wavlm_model.encoder.dropout.p = 0.0
        print("[disable_wavlm_encoder_dropout] - Disabled encoder global dropout.")

    for i, layer in enumerate(wavlm_model.encoder.layers):
        # Disable dropout in the attention layer
        if hasattr(layer.attention, "dropout"):
            layer.attention.dropout = 0.0
            print(f"[disable_wavlm_encoder_dropout] - Disabled dropout in attention for layer {i}.")

        # Disable dropout in feed-forward layers
        if hasattr(layer.feed_forward, "intermediate_dropout"):
            layer.feed_forward.intermediate_dropout.p = 0.0
            print(f"[disable_wavlm_encoder_dropout] - Disabled intermediate dropout for layer {i}.")

        if hasattr(layer.feed_forward, "output_dropout"):
            layer.feed_forward.output_dropout.p = 0.0
            print(f"[disable_wavlm_encoder_dropout] - Disabled output dropout for layer {i}.")

    print("[disable_wavlm_encoder_dropout] - All dropouts in WavLM encoder have been disabled.")
