    ...
    def __init__(
        ...
        **kwargs
        ...
        for x, x_len in zip(X, X_lens):
            ...
            # Fill in batch_x until batch is full
            if len(batch_x) == bucket_size:
                all_librispeech = all('librispeech' in x.lower() for x in batch_x)
                all_music4all = all('music4all' in x.lower() for x in batch_x)
                
                if all_librispeech or all_music4all:
                    # If the batch contains only librispeech or only music4all, skip this batch
                    print('============ Making Batch Again ============')
                    batch_x, batch_len = [], []  # Reset the batch
                    continue
                # Half the batch size if seq too long
                if (
                    (bucket_size >= 2)
        ...
        #         ############################
        #         if data_indices[i] >= len(table):  # Exhausted dataset
        #             tables_shuffled[i] = table.sample(frac=1).reset_index(drop=True)  # Reshuffle
        #             data_indices[i] = 0
        ...
        #     remaining_slots = bucket_size - len(batch_x)
        #     if remaining_slots > 0:
        #         all_samples = []
        #         for i, table in enumerate(tables_shuffled):
        #             start_idx = data_indices[i]
        #             end_idx = min(start_idx + remaining_slots, len(table))
        #             all_samples.extend(
        #                 zip(
        #                     table["file_path"].iloc[start_idx:end_idx],
        #                     table["length"].iloc[start_idx:end_idx],
        #                 )
        #             )
        #             data_indices[i] = end_idx

        #         # Shuffle all available samples and fill remaining slots
        #         random.shuffle(all_samples)
        #         for sample in all_samples[:remaining_slots]:
        #             batch_x.append(sample[0])
        #             batch_len.append(sample[1])
